...
public class DeckDetailPanel {
    ...
    private static VBox buildContent(DeckData dd) {
        ...
        leftInfo.getChildren().addAll(
                infoLabel("ID: " + dd.deck().getDeckID()),
                infoLabel("Cards: " + dd.cardCount()),
                infoLabel("Created at: " + dd.deck().getCreatedAt().format(fmt)));

        VBox rightInfo = new VBox(6);
        Label descTitle = new Label("Description:");
        descTitle.setFont(Font.font("Serif", 14));
        String descText = dd.deck().getDescription();
        ...
        ProgressBar bar = new ProgressBar(dd.progressPercent() / 100.0);
        bar.setMaxWidth(Double.MAX_VALUE);
        bar.setPrefHeight(36);
        bar.setStyle("-fx-accent: " + HEADER_BLUE + ";");

        Label pctLbl = new Label(dd.progressPercent() + "%");
        ...
        List<Flashcard> preview = dd.cards();
